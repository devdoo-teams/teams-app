...
## 현재 run 52 — normalized list succeeded, but the diagnostic receipt masked nested state

- Source/release: `39aa6f5b9ef90cb45ecae585e40e741ebf27eaed`, app `1.0.103`, immutable GitHub handoff and hosted checkout matched exactly; Azure CLI `2.89.1`, Azure DevOps extension `1.0.7`, Bicep `0.46.1`.
- Outcome: `FAIL_AFTER_APPROVAL` at `revision-and-health`. Run 52 / build `20260907.8` passed handoff, hosted Core, RBAC, approval, workload what-if, Blob staging, and workload mutation, then exhausted the bounded revision poll. The worker-runtime probe was not reached.
- Exact hosted evidence: task log 44 recorded repeated `Azure revision list response normalized: array (shape="array")` messages from `18:29:47Z` through `18:35:25Z`, then failed for `teamsapp-canary-goictvxm--39aa6f5b9e`. The normalized list found the expected revision name, but the retained candidate fields were all `null`; the build summary recorded `boundary=revision-and-health exitCode=1` and failure receipt SHA `add6d495e1a20aea59e9da1c336e016e94b6ec637556b47a3c47fe628568580f`.
- Classification: `CONFIRMED_FAILURE_BOUNDARY / RECEIPT_DIAGNOSTIC_BUG`; the actual Azure revision state at the poll time remains `ROOT_CAUSE_REVIEW_REQUIRED`. The null fields cannot be treated as Azure's raw response because the previous jq receipt expression read `active`, `provisioningState`, `runningState`, `healthState`, `trafficWeight`, and `replicas` from the resource root instead of `properties.*`. The list normalizer itself was exercised as a top-level array, so the Run 51 `RevisionCollection.value` hypothesis was not the observed Run 52 shape.
- Official basis: Azure's current Container Apps revision contract places those readiness fields below `Revision.properties` and defines the list collection separately as `RevisionCollection.value`: [Azure CLI revision commands](https://learn.microsoft.com/en-us/cli/azure/containerapp/revision?view=azure-cli-latest), [Container Apps revision list REST schema](https://learn.microsoft.com/en-us/rest/api/resource-manager/containerapps/container-apps-revisions/list-revisions?view=rest-resource-manager-containerapps-2026-01-01).
- Fix prepared: replace the untested jq receipt projection with the snapshotted `scripts/azure-revision-readback.mjs` receipt helper; add RED/GREEN coverage for nested properties, single `revision show`, exact response-shape text, and malformed diagnostics; add a pipeline contract assertion rejecting root-level shorthand. No version bump, package upload, or Teams completion message.
- Decision: retain Run 52 as failed. Commit and push the diagnostic fix, run the clean Azure Core gate, create a fresh immutable handoff, and perform one bounded hosted rerun. Until a hosted run proves revision readiness, the worker runtime, 24/7, A2A, portal, desktop, and mobile gates remain blocked or unverified.

## 현재 run 53 — observed `RunningAtMaxScale` state was not allowlisted

- Source/release: `18d20a7baa2770118d6529dca802a87702e23e26`, app `1.0.103`, immutable GitHub handoff and hosted checkout matched exactly; Azure CLI `2.89.1`, Azure DevOps extension `1.0.7`, Bicep `0.46.1`.
- Outcome: `FAIL_AFTER_APPROVAL` at `revision-and-health`. Handoff, hosted Core `30/30`, RBAC, approval, workload what-if, Blob staging, and workload mutation passed; worker-runtime was not reached.
- Corrected receipt: workload artifact `316` contained `azure-revision-state.json` (332 B). Existing authenticated Ego Lite read-back returned `active=true`, `provisioningState=Provisioned`, `runningState=RunningAtMaxScale`, `healthState=Healthy`, `trafficWeight=100`, and `replicas=1` for `teamsapp-canary-goictvxm--18d20a7baa`.
- Exact failure: the strict readiness predicate did not accept `RunningAtMaxScale`; task log 50 recorded `boundary=revision-and-health exitCode=1`, failure receipt artifact `317` was 541 B, and task receipt SHA was `041423bce6582666dd193634e303a9ed683b79561f67c55516de2e626c0fc77b`.
- Classification: `CONFIRMED_ROOT_CAUSE / REVISION_READINESS_ALLOWLIST_MISSING_OBSERVED_RUNNING_AT_MAX_SCALE`. The prior receipt projection defect is fixed. Current Microsoft REST documentation does not enumerate this observed state, so it is also `CONTRACT_DRIFT_REVIEW_REQUIRED`; this is not a generic permission to accept unknown values.
- Fix prepared: accept only the exact observed state when healthy and `replicas>=1`, while retaining active/provisioned/100%-traffic checks; reject zero-replica or unhealthy variants. Add RED/GREEN coverage and update the shared contract plus hosted predicates. Keep version `1.0.103`; no Teams upload or completion message.
- Decision: commit/push the strict correction, create a fresh immutable handoff, and run one bounded hosted verification. Only then can the VM worker, 24/7, A2A, portal, desktop, and mobile gates proceed.

## 현재 run 54 — worker runtime auth is missing on the Azure VM

- Source/release: `319e676d13b3b366a1df52bee022306b71da0eac`, app `1.0.103`, image `sha256:5a041806...`; Azure CLI `2.89.1`, Azure DevOps extension `1.0.7`, Bicep `0.46.1`.
- Azure DevOps build `20260908.1` read-back: handoff, hosted Core/RBAC, environment approval, workload what-if, Blob staging, workload deployment, revision readiness, and the public health fetch passed far enough to invoke the VM probe.
- Outcome: `FAIL_AFTER_APPROVAL` at `worker-runtime`; `az vm run-command invoke --command-id RunShellScript` returned to the hosted task, but the probe verifier failed with `Invalid Azure worker runtime probe: auth_file was "missing"; expected "present"`.
- Source contract: `azure-pipelines.yml:883-906` invokes the probe after health. `scripts/azure-worker-runtime-probe.mjs:109-150` reads only metadata for `$AGENT_CODEX_HOME/auth.json` and requires regular/600/nlink1/teamsworker plus authenticated `codex login status`.
- Receipts: failure artifact `325` was listed at `536 B` (`471 B` JSON plus `65 B` SHA sidecar); workload artifact `324` was listed at `53 KB`. The task logged `receiptWriteStatus=READY; receiptSha256=447d817af8f49a87ccd8f32057ac474e3d9f8cd39b1218f028da06c5012180be`. The raw failure JSON was not promoted from the artifact redirect in this read-back, so only the task log and artifact metadata are used here.
- Classification: `CONFIRMED_FAILURE_BOUNDARY / WORKER_AUTH_OUT_OF_BAND_MISSING`; the exact missing-file observation is confirmed, but the reason the VM file is absent is `ROOT_CAUSE_REVIEW_REQUIRED`. Mac lock/CUA state is unrelated to the Azure VM credential state.
- Decision: `AZURE_CANARY_REVISION_PASS / PUBLIC_HEALTH_FETCH_PASS / WORKER_RUNTIME_GATE_BLOCKED / RELEASE_BLOCKED`. No app version bump, Teams upload, traffic promotion, or completion message.
- Next: user-only device login on the existing Azure VM, without copying or logging credential contents; then one bounded probe read-back. Official Run Command contract: https://learn.microsoft.com/en-us/azure/virtual-machines/linux/run-command
