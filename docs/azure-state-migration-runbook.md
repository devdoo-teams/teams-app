...
The repository contains no signer, private key, or protected live Azure/ADO adapter, and does not claim that constructing `DefaultAzureCredential` proves a live Azure request. The typed `AzureOperationalEvidenceVerifierProvider` boundary is private-capability based: only a separately reviewed deployment adapter can register a provider, and its immutable policy plus durable challenge store must remain outside the evidence caller. The exported local verifier factory is a deterministic test fixture; it encloses its copied target/trust/challenge policy behind that boundary and consumes a valid challenge through an atomic local marker. Generated-key tests can therefore prove `LOCAL_SIGNED_VERIFIER_CONTRACT_PASS` or the non-production `PROTECTED_OPERATIONAL_VERIFIER_CONTRACT_PASS`, never `READY`. The production `validateAzureIntegratedEvidence` entry point has no caller-accessible capability injection and remains fail-closed as `AZURE_LIVE_EVIDENCE_UNVERIFIED` until a separately reviewed protected Azure/ADO adapter is supplied at deployment time.
