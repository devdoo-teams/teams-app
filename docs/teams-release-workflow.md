...
revision readiness는 `Running`만으로 판정하지 않는다. Azure Container Apps 공식 문서는 `Scale to 0`을 replica 0의 정상 running 상태로 정의하고, 최소 replica 0을 허용하며, 항상 실행할 instance가 필요하면 `minReplicas >= 1`을 사용하라고 명시한다. 따라서 HTTP canary는 `.properties.provisioningState`가 공식 `Provisioned`(기존 호환 fixture의 `Succeeded`도 허용), active, traffic 100%이고 `runningState`가 `Running`일 때 통과시킨다. 현재 Azure 서비스가 Run 53에서 반환한 `RunningAtMaxScale`은 공식 REST enum에 명시되지 않은 관찰된 provider 상태이므로, 일반적인 문자열 허용으로 처리하지 않고 `healthState == Healthy`와 `replicas >= 1`을 함께 확인할 때만 엄격히 허용하며 `CONTRACT_DRIFT_REVIEW_REQUIRED`로 기록한다. `ScaledToZero`는 `Healthy`일 때만 HTTP canary에 허용되며 24/7 상시 프로세스 증거가 아니다. 24/7 승격은 별도로 `minReplicas >= 1`, worker VM service/heartbeat/restart recovery, durable worker receipt를 read-back한다. 근거: [Container Apps scaling](https://learn.microsoft.com/en-us/azure/container-apps/scale-app), lines 31-56, [Container Apps revisions](https://learn.microsoft.com/en-us/azure/container-apps/revisions), lines 48-72 and 128-138, 및 [Container Apps revision REST schema](https://learn.microsoft.com/en-us/rest/api/resource-manager/containerapps/container-apps-revisions/list-revisions?view=rest-resource-manager-containerapps-2026-01-01).

worker runtime은 ACA HTTP health와 별도 gate다. workload deployment와 public `/api/health`가 통과한 뒤에도, Azure VM Run Command의 비대화형 `RunShellScript` probe가 systemd enabled/active/running, 설치 release/manifest commit, Codex executable digest, owner-only regular `auth.json` 메타데이터, `teamsworker` 권한으로 실행한 `codex login status`를 모두 확인해야 한다. Microsoft는 Linux VM Run Command가 VM agent를 통해 `RunShellScript`를 실행하고 interactive prompt를 지원하지 않는다고 명시한다 ([Run scripts in a Linux VM by using action Run Commands](https://learn.microsoft.com/en-us/azure/virtual-machines/linux/run-command), Benefits, Restrictions, Available commands, Azure CLI). probe가 실패하면 `worker-runtime` boundary와 secret-free failure receipt만 남기고 release/Teams 완료를 중단한다. Authenticator/MFA/device login은 이 probe가 수행하지 않으며 사용자-only handoff다.

post-approval AzureCLI 실패 보존은 `ERR` trap에 의존하지 않는다. receipt writer와 임시 파일 정리를 하나의 nonzero `EXIT` trap으로 묶어 명시적 `exit 1`도 원래 exit code와 함께 JSON receipt·SHA-256 sidecar로 보존한다. 성공 시에는 failure receipt를 만들지 않는다. receipt가 non-empty/schema-valid/checksum-backed로 artifact에 남고 실제 bytes가 read-back되지 않으면 실패 원인은 `UNVERIFIED`다. 이 규칙은 Run 40에서 드러난 explicit-exit 경로를 `/bin/bash` 회귀 테스트로 고정했다. 근거: [Azure Pipelines deployment jobs](https://learn.microsoft.com/en-us/azure/devops/pipelines/process/deployment-jobs?view=azure-devops), lines 55-76.
