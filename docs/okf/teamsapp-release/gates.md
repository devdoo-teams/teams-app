...
Run 52 is the current Azure canary result: it used the exact `39aa6f5` handoff and hosted checkout, passed the handoff, hosted Azure Core/RBAC, manual approval, workload what-if, Blob staging, and workload mutation, then failed at `revision-and-health`. The normalized list was observed as a top-level `array` and contained the expected revision name, but the readiness predicate did not pass. The receipt's all-null fields were additionally identified as a diagnostic projection bug because the old jq shorthand read the resource root instead of `revision.properties.*`; the raw provider body remains `ROOT_CAUSE_REVIEW_REQUIRED`. The nested-property receipt fix is locally RED/GREEN verified but not yet hosted-verified. Worker-runtime, Teams package, desktop/mobile, and A2A promotion remain `BLOCKED` or `UNVERIFIED`; no app version bump, upload, or completion message is allowed.
