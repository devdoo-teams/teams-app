...
For a Teams SDK app that combines a bot and a tab, `webApplicationInfo.id` identifies the Entra app that issues the tab token, while `webApplicationInfo.resource` must be the observed combined bot-and-tab resource URI `api://<TAB_DOMAIN>/botid-<BOT_CLIENT_ID>`. Configure that URI, the `access_as_user` scope, Teams Web/Desktop preauthorization, and the Bot Framework redirect URI on the bot Entra app before uploading. Include both the tab host and `token.botframework.com` in `validDomains`. The local MVP can be runtime-verified without sideloading by using `TEAMS_SKIP_AUTH=true` and the HTTP checks documented in the project README.
