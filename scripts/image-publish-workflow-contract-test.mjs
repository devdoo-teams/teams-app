...
assert.doesNotMatch(workflow, /github\.event\.repository\.visibility/, 'provenance must fail closed instead of silently skipping unsupported repository plans');
...
requireText(/subject-path:\s*dist\/evidence\/azure-release-receipt\.json/, 'the release receipt consumed by Azure must be attested');
...
const receiptAttestationStep = workflow.indexOf('      - name: Attest the Azure release receipt');
const packageAttestationStep = workflow.indexOf('      - name: Attest the Teams package bound by the receipt');
...
assert.ok(receiptAttestationStep >= 0, 'promotion must attest the exact Azure receipt');
assert.ok(packageAttestationStep >= 0, 'promotion must attest the exact Teams package');
...
assert.ok(attestationStep < receiptAttestationStep, 'the OCI image must be attested before the receipt handoff');
assert.ok(receiptAttestationStep < packageAttestationStep, 'receipt and Teams package attestations must both complete before upload');
assert.ok(packageAttestationStep < identityUploadStep, 'the handoff artifact must not publish until all attestations succeed');
