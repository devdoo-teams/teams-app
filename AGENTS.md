...
5a. Azure Linux worker를 사용하는 배포는 ACA HTTP health 뒤에 별도의 `worker-runtime` gate를 실행한다. 기존 VM에 대해 Azure VM Run Command의 비대화형 `RunShellScript` probe가 systemd/service, exact release/manifest commit, Codex executable SHA-256, owner-only regular `auth.json` 메타데이터, `teamsworker` 계정의 `codex login status`를 모두 검증해야 한다. Authenticator/MFA/device login은 사용자가 수행하며, probe는 auth contents를 읽거나 복사하지 않는다. 이 gate가 없거나 실패하면 24/7/A2A/완료보고를 주장하지 않는다.
...
- Atlassian/Jira 작업은 연결된 `Atlassian Rovo` MCP를 항상 먼저 조사하고 사용한다. 중복 검색, 사용자·site·project 확인, 이슈 생성·수정·댓글·workflow transition은 MCP의 실제 read/write 응답으로 수행하며, MCP가 현재 세션에 없거나 실제 호출이 실패한 경우에만 그 blocker를 먼저 보고한 뒤 기존 로그인 인앱 브라우저 탭을 사용한다. MCP 가용성을 확인하지 않은 채 Jira UI 입력부터 시작하지 않는다.
...
- 이 저장소의 현재 Git `origin`은 `https://github.com/devdoo-teams/teams-app.git`이다. Bitbucket은 선택적인 추가 remote일 뿐이며, 인증된 Bitbucket 화면에서 workspace·repository slug·visibility·clone URL을 확인하기 전에는 추론·추가하지 않는다. 이후 PR이 필요해지면 저장소 설정이 명시적으로 바뀌지 않은 한 현재 구성된 GitHub `origin` 워크플로우를 사용한다.
...
- `release:public`은 명시적 `--url` 다음 `TEAMS_PUBLIC_URL`, `PUBLIC_BASE_URL`, `.env.runtime`의 `TAB_DOMAIN`에서 공개 origin을 해석한다. core 게이트는 `typecheck:core`의 bounded esbuild CLI stdin transform과 `build:core`의 React/Teams SDK/Codex 경로를 사용한다. `typecheck:core`는 workspace tsconfig auto-discovery를 끄고 long-lived service mode를 사용하지 않는다. 전체 `typecheck`는 현재 Node 24/TypeScript 선언 그래프에서 장시간 대기할 수 있으므로 별도 진단으로 실행하며, core source check를 semantic TypeScript 전체 통과로 과장하지 않는다. vendor 선언 그래프 진단은 `typecheck:vendor`로 분리한다.
...
- 원격 화면과 잠금 판정은 [docs/teams-release-workflow.md의 원격 화면·잠금·캡처 증거 판정](docs/teams-release-workflow.md#원격-화면잠금캡처-증거-판정)을 단일 기준으로 사용한다. `pmset -g assertions`/Caffeine은 `POWER_ASSERTION_ACTIVE`(절전·디스플레이 절전 방지)일 뿐 잠금·잠금 해제의 증거가 아니다. 로컬 `screencapture`가 검은 프레임을 반환하면 `REMOTE_CAPTURE_UNAVAILABLE`, CUA가 잠금 오류를 반환하면 `CUA_CONTROL_UNAVAILABLE`로 기록하며, 둘 중 하나만으로 사용자의 원격 화면이 잠겼다고 말하거나 잠금 해제를 요청하지 않는다. 사용자가 제공한 원격 데스크톱 스크린샷은 캡처 시각의 `USER_REMOTE_VIEW` 증거로 기록하고, 현재 상태로 과장하지 않는다. 서로 다른 호스트·세션·시각의 신호가 충돌하면 `REMOTE_SESSION_MISMATCH`로 분리하고 기존 Ego 탭·CLI 검증을 계속한다.
- 잠금 해제 요청은 같은 호스트·세션에 결합된 직접 잠금 화면/세션 상태와 독립 신호가 모두 있을 때만 한다. 그 전에는 `pmset`, CUA 오류, 검은 캡처를 조합해 잠금이라고 추론하지 말고 명령어 게이트·기존 Ego DOM·공개 HTTP를 우선 진행한다. Caffeine/원격 연결/잠금 설정을 임의로 바꾸거나 끊어서 캡처 문제를 우회하지 않는다.
