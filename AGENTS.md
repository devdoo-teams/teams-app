...
6. 공개 HTTPS URL과 Teams 호스트에서 새 버전의 UI·핵심 명령·변경 기능을 런타임 검증한다. 그 다음 반드시 사용자에게 배포된 Teams 앱(모바일 또는 실제 Teams 호스트)에서 테스트 메시지를 직접 보내도록 하고, 사용자가 받은 Bot 응답·GenUI/Adaptive Card·필요한 proactive 진행·완료 메시지를 확인한다. 오케스트레이터가 API나 테스트 하네스에서 만든 요청만으로 사용자 검증을 대체하지 않는다.
