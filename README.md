...
1. Teams Admin Center의 기존 앱 상세에 업로드된 v1.0.14 앱을 Teams 모바일에서 새로 고침하거나 앱을 다시 추가하고 `업무 허브` 개인 탭을 엽니다.
...
`npm test`에는 Task 0–7의 저장소 hardening, 결정형/OpenAI/local provider, response-mode 저장소·API, Teams 탭 selector, MCP GenUI, 기존 CopilotKit·Teams·ACL 런타임 검증이 모두 포함됩니다. 릴리스 assertions는 무키 provider 상태, 결정형 기본값, 응답 모드 인증 경계, MCP/Teams/CopilotKit의 공통 GenUI 계약, v1.0.14 런타임 버전을 확인합니다.
...
이 릴리스 후보의 소스 package와 Teams manifest 버전은 `1.0.14`으로 고정되어 있으며 `npm run validate:manifest`가 두 값을 함께 검사합니다. 실제 배포 환경값이 없는 상태에서는 패키지를 업로드하거나 placeholder를 운영 자격 증명으로 간주하지 않습니다.

운영 패키지를 만들기 전에 배포 환경 사전검사를 실행합니다. 이 검사는 검증용 placeholder, 로컬 호스트, 잘못된 GUID를 차단합니다. `BOT_ID`는 메시징용 Teams/Bot 등록 ID이며, 봇과 탭을 함께 사용하는 Teams SDK 앱에서는 `APPLICATION_ID_URI`가 봇 클라이언트 ID를 가리키는 `api://botid-<BOT_CLIENT_ID>`여야 합니다. `webApplicationInfo.id`는 별도 인증 앱일 수 있지만 resource URI 계약은 봇 리소스와 일치해야 합니다.
...
ZIP을 새로 만든 뒤에는 내부 `manifest.json`의 버전 `1.0.14`, `devicePermissions: ["geolocation"]`, 실제 valid domain, 해석된 ID/URI를 확인하고 SHA-256을 기록합니다. 이전 ZIP을 재사용하지 않습니다.
...
Teams SDK에서 봇과 탭을 함께 사용하는 SSO는 `TAB_DOMAIN`의 공개 HTTPS 조건과 별개로, 봇 Entra 앱의 Application ID URI·`access_as_user` 범위·Teams Web/Desktop 사전 승인을 사용합니다. 따라서 `APPLICATION_ID_URI`와 manifest의 `webApplicationInfo.resource`는 반드시 `api://botid-<BOT_CLIENT_ID>`와 같아야 하고, `webApplicationInfo.id`는 실제 토큰을 발급하는 인증 앱 ID와 일치해야 합니다. Dev Tunnel의 임시 호스트는 탭 `contentUrl`/valid domain에만 사용하며 resource URI에 넣지 않습니다. SSO 오류가 나면 Bot Entra 앱 설정과 Teams 앱 매니페스트를 함께 확인한 뒤 버전 증가·새 패키지 생성·재업로드를 수행합니다.
